...
export function compact(
  ...
  if (compactType === "inline") {
    ...
    //       if (item.h < maxHeight) {
    //         console.log(`updating height of ${item.i} to ${maxHeight}`);
    //         item.h = maxHeight;
    //       }
    //     });
    //   } else {
...
export function compactItem(
  ...
  if (compactV) {
    ...
    console.log(JSON.parse(JSON.stringify(sameColumnItems)));
    console.log(`moving ${l.i}: bottomY=${bottomY}, l.y=${l.y} to ${Math.min(bottomY, l.y)}`, map(compareWith, 'i').join(','));
    l.y = Math.min(bottomY, l.y);


    // Move up as far as possible within same column
    while (l.y > 0 && !getSameColumnCollision(compareWith, l)) {
      console.log('decreasing y of', {i: l.i, y: `${l.y} -> ${l.y - 1}`, compare: JSON.stringify(compareWith)});
      l.y--;
    }
    ...
    console.log("// =================== sameRowItems ================= //", sameRowItems)
    if (sameRowItems.length > 0) {
      ...
      if (l.h < maxHeight) {
        ...
        console.log("// =================== sameRowItems : Resized item to max height ================= //", l)
      }


    } else {
      if (l.originalH) {
        console.log("// =================== sameRowItems : Resetting height to originalH ================= //", l)
        l.h = l.originalH
      }
    }

    console.log("// =================== END ================= //", fullLayout)
  }
...
function getSameColumnCollision(
  ...
  for (let i = 0, len = layout.length; i < len; i++) {
    if (
      layout[i].x === layoutItem.x && // Same column
      collides(layout[i], layoutItem) && // Collides
      layout[i].w <= layoutItem.w // Ensure the width of the colliding item is not greater
    ) {
      return layout[i];
    }
  }
