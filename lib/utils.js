...
export function compact(
  ...
  if (firstRun && compactType === "inline") {
...
export function compactItem(
  ...
  if (compactV) {
    ...
    const bottomY = sameColumnItems.length > 0
      ? Math.max(...sameColumnItems.map(item => item.y + item.h))
      : 0;
    ...
    if (sameRowItems.length > 0) {
      ...
      //   if (item.h < maxHeight) {
      //     // Update the item in the full layout
      //     const layoutItem = compareWith.find(li => li.i === item.i);
      //     if (layoutItem) {
      //       layoutItem.h = maxHeight;
      //     }
      //   }
      // });
    } else {
