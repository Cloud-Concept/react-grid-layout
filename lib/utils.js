...
export function compactItem(
  ...
  if (compactV) {
    ...
    const sameColumnItems = fullLayout.filter(
      item => item.x === l.x || item.w > 1
    );
    const bottomY = bottom(sameColumnItems);
    log(JSON.parse(JSON.stringify(sameColumnItems)));
    log(
      `moving ${l.i}: bottomY=${bottomY}, l.y=${l.y} to ${Math.min(
        bottomY,
        l.y
      )}`,
      map(compareWith, "i").join(",")
    );
    ...
    while (l.y > 0 && !getSameColumnCollision(compareWith, l)) {
      log("decreasing y of", {
        i: l.i,
        y: `${l.y} -> ${l.y - 1}`,
        compare: JSON.stringify(compareWith)
      });
      l.y--;
    }
    if (sameRowItems.length > 0) {
      ...
      const maxHeight = Math.max(
        ...sameRowItems.map(item => item.originalH || item.h),
        l.originalH || l.h
      );
      ...
      if (l.h < maxHeight) {
        ...
        log(
          "// =================== sameRowItems : Resized item to max height ================= //",
          l
        );
      }
    } else {
      if (l.originalH) {
        log(
          "// =================== sameRowItems : Resetting height to originalH ================= //",
          l
        );
        l.h = l.originalH;
      }
...
function getSameColumnCollision(
  ...
  for (let i = 0, len = layout.length; i < len; i++) {
    ...
    const itemOverlapsColumns =
      (layout[i].x >= layoutItem.x &&
        layout[i].x < layoutItem.x + layoutItem.w) || // Item starts within layoutItem's width
